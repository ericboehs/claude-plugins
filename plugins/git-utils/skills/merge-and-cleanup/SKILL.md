...
description: Merge the current branch's PR (squash) and clean up — delete branch, switch to main, pull, remove worktree if applicable. Use when user says "/merge-and-cleanup", "merge and cleanup", or "merge this PR".
tools: Bash
---
...
- If the PR state is not `OPEN`, tell the user the PR is already closed/merged.
...
```bash
# Get the worktree path we're currently in
WORKTREE_PATH=$(pwd)

# Get the main repo path (parent of .git common dir)
...
5. If anything fails, stop and report clearly — don't try to continue partial cleanup
